...
if [ ${num_nodes} -gt ${num_gpus} ]; then
    echo "WARNING: found ${num_nodes} NUMA nodes but only ${num_gpus} GPUs, reducing number of NUMA nodes to number of GPUs. We highly encourage using manual binding (i.e., --cpu=*.sh --mem=*.sh) on this system!"
    num_nodes=${num_gpus}
fi
